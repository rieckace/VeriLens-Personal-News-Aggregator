...
export default function NotificationsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        <div className="rounded-3xl border border-slate-900 bg-slate-900/30 p-8 text-slate-300">
          No notifications yet.
        </div>
      ) : (
        <div className="overflow-hidden rounded-3xl border border-slate-900 bg-slate-900/20">
          <ul className="divide-y divide-slate-900">
            {items.map((n) => (
              <li key={n._id}>
                <button
                  onClick={() => onOpen(n)}
                  className={`flex w-full items-start justify-between gap-4 px-5 py-4 text-left transition hover:bg-slate-900/40 ${
                    n.isRead ? 'opacity-80' : 'bg-slate-950/20'
                  }`}
                >
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 line-clamp-2 text-sm text-slate-300">{n.message}</div>
                    {n.createdAt ? (
